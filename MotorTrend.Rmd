...
```
Below is a plot of mpg aganist type of transmission. We can see that the maximum mileage/gallon for automatic is around `r max(mtcars$mpg[which(mtcars$automatic == TRUE)])` and for manual it is around `r max(mtcars$mpg[which(mtcars$automatic == FALSE)])`.
